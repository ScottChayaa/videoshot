...
**目前進度：階段 0～3 完成，階段 4a（精靈外殼與第一步）完成（2026-09-14）、階段 4b（縮圖牆與收斂）、階段 4c（截圖與效能閘門）完成（2026-09-15）。共 332 個測試（JVM 244 ＋ 實機儀器 88）。**

**app 啟動後直接落在取圖精靈**（階段 2 的資料層冒煙畫面已刪除，內容在 git 歷史）。
貼網址 → 挑畫面 → 截圖補洞這一段在實機上可以真的操作；**第三步仍是佔位文字**，
所以圖還進不了 `library.db` —— 那是階段 6。
...
下一步是**階段 6**（第三步、完成、草稿 —— 做完才能真的把圖存進圖庫）或**階段 7**（外殼、首頁、視覺設計）；**階段 14**（清理 web 程式碼）也隨時可做。**原本的階段 5 已併入 4c**。
