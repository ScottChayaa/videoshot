...
**目前進度：階段 0～3 完成，階段 4a（精靈外殼與第一步）完成（2026-09-14）、階段 4b（縮圖牆與收斂）完成（2026-09-15）。共 268 個測試（JVM 180 ＋ 實機儀器 88）。**

**app 啟動後直接落在取圖精靈**（階段 2 的資料層冒煙畫面已刪除，內容在 git 歷史）。貼網址 → 挑畫面這一段在實機上可以真的操作；第三步仍是佔位文字。
...
下一步是**階段 4c**（【截圖】與效能閘門，細節計畫已產出、原階段 5 已併入）或**階段 7**（外殼、首頁）；**階段 14**（清理 web 程式碼）也隨時可做。
