...
**目前進度：階段 0～3 完成，階段 4a（精靈外殼與第一步）完成（2026-09-14）、階段 4b（縮圖牆與收斂）、階段 4c（截圖與效能閘門）完成（2026-09-15）、階段 6（第三步、完成、草稿）、階段 7（App 外殼、首頁、Lightbox）完成（2026-09-16）、階段 8（分類資料夾）完成（2026-09-23）。
三套測試全綠：JVM **535 個**（`:core:test` 130 ＋ `:app:testDebugUnitTest` 405，2026-09-23），
**儀器測試 143 個**（`:app:connectedDebugAndroidTest`，實機 2107113SG，2026-09-23 Task 10 驗收那一次；
本階段的細節計畫已把資料夾寫入的連動——加入、移出、改名、刪除——都補進儀器測試）。
