...
**sheet 的副檔名是 `.jpg`，內容其實是 WebP**（`RIFF` 開頭，2026-09-15 實測）。
Android 的 `BitmapFactory` 兩種都吃得下所以 app 不受影響，但**離開 Android 的工具就會踩到** ——
Java 的 `ImageIO` 解不了 WebP，而且是安靜地回傳 null，不拋例外。
...
└── drafts/{videoId}/
    ├── sheets/L{level}/M{n}.jpg                    # 取圖用的 sheet，綁在草稿上
...
│           ⋮                │     灰＋鎖＝先前已收藏
├───────────────────────────┤
│ 96 張候選 · 已選 18   [下一步（18 張）] │  ← 只有一列，底下不疊導覽列
└───────────────────────────┘
```
...
縮圖牆不是攤開所有格子讓使用者自己找，而是**一進第二步就收斂成候選**，把連續相似的畫面藏起來。

- 頂部狀態列：「已收斂成 N 張候選，隱藏了 M 張相似畫面」＋【顯示全部】。
  按下後全部出現，按鈕變成【重新過濾】，可以來回切。
- **隱藏張數完全取決於內容，經常是 0**。storyboard 的取樣間隔由 YouTube 決定（2／5／10 秒），
  對剪接快的影片來說，相鄰格本來就是不同場景 —— 沒有冗餘可收斂。**這是正確行為，不是故障**：
  2026-09-15 實測一支 21 分鐘的美食 vlog（127 格、10 秒間隔），相鄰格的漢明距離中位數 31，
  而相隔 40 格的中位數是 32 —— 兩者統計上無法區分，且 32 正是 64 位元雜湊在兩張無關圖片之間的期望值。
  反過來，靜態運鏡的內容藏得掉很多（演講 163 → 106、固定機位講話 129 → 51，皆為中強度）。
  狀態列在 M = 0 時只講「N 張候選」，不顯示【顯示全部】。
- **強度沿用帳號頁的「取圖 › 過濾相似強度」**（高／中／低，預設中）。切換設定後，以新門檻**對原始格數重算**，不是在已過濾的結果上疊加。
- 演算法：每格縮到 9×8 灰階，算 **dHash**（64-bit 指紋），漢明距離 **≤ 門檻**視為相似。
  依時間順序掃描，每組保留最早的一張 —— 比較的對象是「最近一張**被保留的**」，不是「前一張」
  （跟前一張比的話，畫面緩慢變化時會一路藏到底，只剩第一張）。
  門檻：**高 ≤ 10、中 ≤ 6、低 ≤ 3**（`:core` 的 `FilterStrength`）。2026-09-15 以五支真實影片實測確認：
  三檔在所有樣本上都單調遞增，且全部高於雜訊底噪，不需要調整。
- **9×8 的每一格取的是該區域所有像素的平均，不是單一像素。** 這是正確性要求，不是精緻化：
  單點取樣時把取樣座標挪一個像素，指紋就差**中位數 7 個位元**（p90 13）—— 比中強度的門檻還大，
  比對到的是 WebP 的區塊雜訊而不是畫面內容，收斂因此幾乎藏不掉任何東西
  （2026-09-15 實機症狀：108 格只藏 1 張）。改成區域平均後底噪降到中位數 1～2 個位元（p90 3～5）。
  取樣數學放在 `:core`（純 JVM，測得完），`:app` 只負責用 `Bitmap.getPixels` 把該格像素整塊搬出來。
- 在收斂算完之前，縮圖牆已經先畫出來、可以捲動與點選；狀態列顯示「正在過濾相似畫面…」。

> **效能閘門（R-3）**：2026-09-15 實測，三支影片（3:33／10:35／26:53）各 5 次的 p75 是
> **803／1004／1046 ms** —— 遠低於 3 秒門檻，**維持進場即收斂**。
> 其中指紋 300～500 ms（約每格 3 ms，含解圖）、收斂本身 50～80 ms，計算約佔一半。
> **總時間與影片長度幾乎無關** —— 縮圖牆的張數本來就與影片長度無關（第二節第 1 點）。
>
> 量測機是 Xiaomi 11T Pro（旗艦）＋ WiFi，**不是** R-3 要求的中階機 ＋ 4G 節流，
> 所以這是下限而非典型值；粗估中階機 ＋ 4G 約 3.7 秒，落在「維持，但狀態列要明確可見」的區間
> （狀態列的「正在過濾相似畫面…」已經滿足）。細節見
> [階段 4c 效能量測](../plans/2026-09-15-階段4c-效能量測.md)。
...
- **預設沒有時間篩選**。右上角日曆鈕開出**月份選擇器**（只列出實際有收藏的月份與張數），選定後出現
  可清除的狀態列「只顯示 ○年○月 以前的收藏 ✕」，清單只留該月（含）以前並回到頂端，該月即第一個分組。
  篩選**下推到 SQL**（`event_date < 下月一日`）而非撈回來再過濾 —— keyset 分頁下用記憶體過濾會讓被濾掉的
  資料佔掉 limit 的名額，游標因此跳過真正該出現的列；**篩選不持久化**，重開 app 回到完整時間軸（手冊 §二第三條）。
...
| 清單頁操作 | 名稱篩選 ＋ 四種排序（名稱升／降、張數多→少、最近加入）；排序偏好存 DataStore（裝置本地，不進備份） |
...
6. 過濾強度高／中／低 → 隱藏張數依序遞減；切換強度是對原始格數重算。**隱藏 0 張是合法結果**（剪接快的影片沒有冗餘），此時不顯示【顯示全部】
...
- 深色模式、鍵盤與輔助技術可操作
- **主題（配色）可切換**：app 內選色系（帳號頁，階段 11 的 UI），偏好存 DataStore。
