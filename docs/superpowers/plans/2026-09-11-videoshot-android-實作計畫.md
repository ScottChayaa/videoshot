...
| **4** | 取圖精靈第一、二步（含播放器、收斂） | L | ✅ 完成（4a／4b／4c 三份細節計畫） |
| 5 | 截圖與相簿選圖 | M | ✅ 已併入階段 4c |
| **6** | 取圖精靈第三步、完成、草稿 | L | ✅ 完成（JVM 測試綠燈；儀器測試未跑，見 [階段 6 計畫](2026-09-15-階段6-第三步完成與草稿.md)） |
| **7** | App 外殼、首頁、Lightbox | L | ✅ 完成（見 [階段 7 計畫](2026-09-16-階段7-外殼首頁lightbox.md)） |
| **8** | 分類資料夾 | M | ✅ 完成（見 [階段 8 計畫](2026-09-18-階段8-分類資料夾.md)） |
| 9 | 詳情頁 | M | 待產出 |
...
**拆成三份細節計畫**（10 個任務、第一個有 UI 的階段，單一計畫過大）：

| 計畫 | 涵蓋 |
|---|---|
| [階段 4a](2026-09-13-階段4a-精靈外殼與第一步.md) | T4.1～T4.4、T4.9 |
| [階段 4b](2026-09-14-階段4b-縮圖牆與收斂.md) | T4.5～T4.7，以及 T4.8 的【全部選取】與【只看已選】。另外把 app 的啟動入口從階段 2 的資料層冒煙畫面換成精靈 —— 沒有這一步，第二步在實機上到不了 |
| [階段 4c](2026-09-15-階段4c-截圖與效能閘門.md) ✅ | T4.8 的【截圖】整條（`capture` 介面、JS canvas、黑畫面判定、廣告偵測、相簿退路）與 T4.10 效能閘門。**原本的階段 5 已併入這一份** |

【截圖】之所以自成一份：它是一個完整的子系統，而且規格第十二節還掛著兩個 ⏳ 未驗證的前提；
抽開之後 4b 就是一份能獨立驗收的「挑圖體驗」。
...
| T4.10 | **效能閘門**：「進入第二步 → 收斂完成」p75 | S | ✅ 2026-09-15 實測 p75 約 1 秒，維持進場即收斂（[量測結果](2026-09-15-階段4c-效能量測.md)） |

### 階段 5 —— （已併入階段 4c）

**2026-09-15 合併。** 原本的 T5.1～T5.4 與階段 4c 的「T4.8【截圖】整條」幾乎完全重疊 ——
同一個子系統被排了兩次。【截圖】是一個完整的子系統，切兩半沒有意義，
全部在 [階段 4c](2026-09-15-階段4c-截圖與效能閘門.md) 做完。

**後續階段不重新編號**（階段 6 仍叫階段 6），避免既有文件的交叉引用一起失效。
...
| # | 任務 | 規模 | 驗收 |
|---|---|---|---|
| T6.1 | 4 欄網格、綠點、快捷列（全選／全不選／反選／未填的） | M | ✅ 2026-09-16 案例 11 |
...
| T6.5 | 完成：`library.db` transaction → `cache.db` → 刪草稿 → 標記有變更 → 回首頁捲到月份 ＋ toast | M | ✅ 2026-09-16 案例 12；手冊 §四第三步最後一條。「導回首頁 → 捲到該月份」已隨階段 7 的 T7.2 接上（Task 7，`WizardViewModel.finished`）。`library.db` 的交易寫入由 `LibraryRepoWriteTest`（儀器測試，16 個案例）覆蓋，2026-09-16 已在實機跑過 |
| T6.6 | 草稿存取與續做詢問（續做時重抓 watch page） | M | ✅ 2026-09-16 案例 13 |

### 階段 7 —— App 外殼、首頁、Lightbox ✅

細節見 [階段 7 計畫](2026-09-16-階段7-外殼首頁lightbox.md)。

| # | 任務 | 規模 | 驗收 |
|---|---|---|---|
| T7.1 | 底部導覽五格（取圖只用顏色區分）、淺色／深色主題、間距與字級尺標、平板 ≥ 600dp 加欄。**同時刪掉 `MainActivity` 裡階段 2 留下的臨時資料層冒煙畫面** | M | ✅ 2026-09-16 Task 1、Task 2；手冊 §零。鍵盤焦點框（`Modifier.focusRing`）2026-09-18 補上並在實機驗過 —— 手冊 §零 的輔助操作與平板加欄同日驗完 |
| T7.2 | 首頁：年月分組固定三欄、keyset 50 筆、月份標籤列（橫向捲動）、日曆篩選與可清除狀態列、月份選擇器篩選 | L | ✅ 2026-09-16 Task 3、Task 4、Task 5、Task 6、Task 7；手冊 §二；接上 `WizardViewModel.finished`，取代 `MainActivity` 的暫時 toast。原「懸浮時間軸」改為月份選擇器篩選，規格第六節已同步修訂（Task 12 Step 1） |
| T7.3 | Lightbox：不透明、第 N / 共 M 張、左右滑動、一次性提示 | M | ✅ 2026-09-16 Task 8；手冊 §三前三條 |
| T7.4 | Lightbox 動作分層：【播放這一段】、加入分類、分享（`ACTION_SEND`）、`⋯` 內編輯與刪除 | M | ✅ 2026-09-16 Task 9、Task 10；手冊 §三第四條；案例 17 |
| T7.5 | 刪除單張的連動（`shot_tag`、`shot_folder`、`shot_image`、縮圖檔、`thumb_state`、最後一張刪 `video`） | S | ✅ 2026-09-16 Task 11；案例 23 |

### 階段 8 —— 分類資料夾 ✅

細節見 [階段 8 計畫](2026-09-18-階段8-分類資料夾.md)。該計畫把四個任務拆成 10 個，
並定案三件規格沒寫死的事（「最近加入」＝含子孫的 `MAX(added_at)`、預覽取最近加入的 4 張、
【加入分類】勾一下寫一次），已寫回規格第六節（Task 10 Step 1）。**三套測試全綠、實機驗收完成**
（2026-09-23，見 Task 10 報告）。

| # | 任務 | 規模 | 驗收 | 對應任務 |
|---|---|---|---|---|
| T8.1 | 資料夾 CRUD：樹狀、深度 5、同層不重名、名稱上限 50 | M | ✅ 2026-09-23 案例 22 | Task 1、Task 3 |
| T8.2 | 清單頁：含子孫計數、預覽拼貼（最多 4 張）、篩選 ＋ 四種排序、`⋯`、空狀態 | M | ✅ 2026-09-23 手冊 §六；案例 20 | Task 2、Task 5、Task 6 |
| T8.3 | 資料夾頁（上半子資料夾、下半本層圖）＋ Lightbox【加入分類】bottom sheet | M | ✅ 2026-09-23 案例 20 | Task 4、Task 7、Task 8、Task 9 |
| T8.4 | 刪除資料夾（連子資料夾與關聯，圖不動） | S | ✅ 2026-09-23 案例 21 | Task 3（外鍵連動）、Task 6（確認框） |
