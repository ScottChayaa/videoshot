...
**Spec:** [`../specs/2026-09-10-videoshot-app-design.md`](../specs/2026-09-10-videoshot-app-design.md) 第二節第 4、5、7 點，第五節【截圖】，第十一節，第十二節。
**路線圖：** [`2026-09-11-videoshot-android-實作計畫.md`](2026-09-11-videoshot-android-實作計畫.md) 階段 0。

## Global Constraints

- POC 的 applicationId：`com.xenyaa.videoshot.poc`；minSdk 26
...
開發機是 Linux。Android Studio（snap，內含 JDK 25）與 SDK 都已安裝：
SDK 在 `~/Android/Sdk`（platform android-37.0、build-tools 36.0.0、platform-tools），license 已接受，
缺的元件 AGP 會自動下載。**系統 PATH 上沒有 java**，所以 shell 裡跑 `./gradlew` 必須自己指定 JDK。

- [x] **Step 1: 安裝 Android Studio** —— 已完成（`/snap/android-studio/current`）。

- [ ] **Step 2: 設定環境變數**

  加進 `~/.bashrc`（或每次開終端機時 export）：
...
- [ ] **Step 1: 👤 以範本建立專案**
  ...
  - Name：`videoshot-poc`
  - Package name：`com.xenyaa.videoshot.poc`
...
- [ ] **Step 4: 建置並安裝範本**
  ...
  Expected: `BUILD SUCCESSFUL`，手機上出現 `videoshot-poc` 並顯示範本的 Hello World。
...
**Files:**
- Create: `poc/android-capture/app/src/main/java/com/xenyaa/videoshot/poc/FrameCheck.kt`
...
- [ ] **Step 4: 檢查規格裡沒有殘留的 POC ⏳**
  ...
  Run: `grep -n "⏳" docs/superpowers/specs/2026-09-10-videoshot-app-design.md`
