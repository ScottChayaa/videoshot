...
```
  ...
  **`coroutinesTest` 必須是 1.9.0，不能拉新** —— Compose BOM 對 `kotlinx-coroutines-core` 下了
  `strictly 1.9.0`，只把 test 升到 1.11.0 會在裝置上炸
  `NoSuchMethodError: runBlockingK$default`（core 沒跟著升）。要升就兩邊一起升。

- [ ] **Step 2: `:app` 套用 KSP 與 Room**

  先在 `android/gradle.properties` 末尾加：

```properties
# AGP 9 的 built-in Kotlin 不允許外掛動 kotlin.sourceSets，而 KSP 需要註冊它產生的原始碼目錄。
# 這是 AGP 錯誤訊息裡指定的解除方式（Room 的註解處理靠 KSP）。
android.disallowKotlinSourceSets=false
```

  沒有這一行，設定階段就會失敗：`Using kotlin.sourceSets DSL to add Kotlin sources is not allowed with built-in Kotlin`。
  另外 `room.schemaLocation` 的值要寫成 `"$projectDir/schemas"`（Kotlin DSL 的字串樣板），
  寫成字面的 `$projectDir` 會讓 KSP 報 `Couldn't write schema file!`。
...
- [ ] **Step 10: 執行測試，確認通過**
  ...
  Run: `cd android && ./gradlew :app:connectedDebugAndroidTest`
  Expected: `BUILD SUCCESSFUL`，4 個測試通過（`LibrarySchemaTest` 3 個 ＋ 範本自帶的 `ExampleInstrumentedTest`）。
  **後面各任務標的儀器測試總數都含範本那一個。**
...
interface SearchDao {
     ...
     * shot_fts 是 callback 用原生 SQL 建的虛擬表，Room 的編譯期驗證看不到它
     * （不加這個註解會在 KSP 階段炸 `no such table: shot_fts`），只能跳過這一個方法的驗證。
     * likeIds 查的是 Room 認得的 shot，驗證保留。
     */
...
- [ ] **Step 6: 寫入實作**
  ...
  **交易一定要用 driver API 的那一套。** `room-ktx` 的 `RoomDatabase.withTransaction` 走舊的
  `SupportSQLiteOpenHelper` 路徑，設了 `setDriver(BundledSQLiteDriver())` 之後那條路沒接上，
  執行期會丟 `Cannot return a SupportSQLiteOpenHelper since no SupportSQLiteOpenHelper.Factory was configured`。
  先在檔案末尾（class 外）加這個小包裝：
...
```
  ...
  需要 `import androidx.room.Transactor`、`import androidx.room.useWriterConnection`。
  下面各方法裡的交易一律寫成 `db.inWriteTransaction { … }`，
  而且**不要在區塊裡用帶標籤的 return**（新 API 沒有同名標籤），改用 `if` 包住。
...
```
  ...
  `db.inWriteTransaction { … }` 之後各加一行 `onChanged()`（放在交易外，交易成功才算變更）。
...
**`CacheRepo` 在本計畫的「檔案結構」列了，但七個任務沒有一個建立它** —— self-review 的三項檢查
（規格覆蓋、佔位符、型別一致）都沒抓到這種「檔案結構列了但沒有任務產出」的漏洞。
這違反 Global Constraints 的「所有 DB 存取只走 repo」：`cache.db` 沒有 repo，
階段 4（草稿）與階段 13（回填）會被迫直接碰 DAO。

補做內容（併入 Task 2.4 的範圍）：`data/repo/CacheRepo.kt` 介面與 `RoomCacheRepo` 實作
（`putThumbStates`／`thumbState`／`thumbsDueForRetry`／`forgetVideoThumbs`／
`saveDraft`／`currentDraft`／`clearDraft`／`clearAll`）、`AppContainer.cacheRepo`，
以及 `CacheRepoTest` 4 個測試。

**日後寫計畫時**：檔案結構列出的每一個檔案，都要能指到一個負責建立它的任務。
